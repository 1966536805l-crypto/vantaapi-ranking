    ...
    <main className="apple-page pb-12 pt-4">
      ...
      <section className="apple-shell py-7">
        <div className="module-hero px-5 py-6">
          ...
          <h1 className="apple-display-title mt-3 max-w-4xl text-4xl">{copyLanguage === "zh" ? "原创英语选择填空题库" : "Original English Question Bank"}</h1>
        ...
        <section className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {originalQuestionPacks.map((pack) => (
            <Link key={pack.slug} href={localizedHref(`/english/question-bank/${pack.slug}`, language)} className="dense-card p-4">
              ...
              <h2 className="mt-2 text-xl font-semibold">{copyLanguage === "zh" ? pack.zhTitle : pack.title}</h2>
              <p className="mt-3 text-sm leading-6 text-[color:var(--muted)]">{copyLanguage === "zh" ? pack.descriptionZh : "Original exam-style multiple choice and fill blank questions"}</p>
