          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <Link href={localizedHref("/", language)} className="flex items-center gap-3">
              ...
              <span>
                ...
                <span className="text-xs text-[color:var(--muted)]">AI tools and learning lab</span>
              </span>
            </Link>
            ...
            <nav className="flex flex-wrap items-center gap-2 text-sm">
              ...
              {user?.role === "ADMIN" && <Link href={localizedHref("/admin", language)} className="apple-pill border-amber-200 bg-amber-50/80 px-3 py-1.5 text-amber-800">Admin</Link>}
              {user ? (
                <form action="/api/auth/logout" method="post"><button className="apple-button-secondary px-3 py-1.5">Logout</button></form>
              ) : (
                <Link href={localizedHref("/login", language)} className="apple-button-primary px-4 py-1.5">Login</Link>
              )}
            </nav>
          </div>
        </header>
