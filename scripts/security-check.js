...
if (has(githubSecurityWorkflow, "Security baseline") && has(githubSecurityWorkflow, 'DATABASE_URL: "postgresql://') && has(githubSecurityWorkflow, "npm run security:check") && has(githubSecurityWorkflow, "npm run content:check") && has(githubSecurityWorkflow, "npm run build") && has(githubSecurityWorkflow, "npm run language:smoke")) {
  add("pass", "ci:security-baseline", "GitHub CI runs security checks, content strategy checks, a Postgres-shaped env, production build, and multilingual smoke checks");
} else {
  add("warn", "ci:security-baseline", "GitHub CI security baseline, content strategy check, Postgres env, production build, or multilingual smoke check is missing");
}
